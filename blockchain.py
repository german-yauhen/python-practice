    ...
    with open('blockchain.txt', mode='r') as f:
        ...
        for block in blockchain_read:
            restored_block = {
                ...
                'proof': block['proof'],
                'transactions': [
                    OrderedDict([('sender', trx['sender']), ('recipient', trx['recipient']), ('amount', trx['amount'])])
        ...
        for trx_read in open_transactions_read:
            trx_restored = OrderedDict([('sender', trx_read['sender']), ('recipient', trx_read['recipient']), ('amount', trx_read['amount'])])
...
def get_balance(participant):
    sent_amounts = [[tx['amount'] for tx in block['transactions'] if tx['sender'] == participant] for block in blockchain]
    sent_amounts_open = [tx['amount'] for tx in open_transactions if tx['sender'] == participant]
    sent_amounts.append(sent_amounts_open)
    total_sent = reduce(lambda sent_sum, sent_amount: sent_sum + sum(sent_amount) if len(sent_amount) > 0 else sent_sum + 0, sent_amounts, 0)
    # for sent_amount in sent_amounts:
    #     if len(sent_amount) > 0:
    #         total_sent += sent_amount[0]
    
    received_amounts = [[tx['amount'] for tx in block['transactions'] if tx['recipient'] == participant] for block in blockchain]
    total_received = reduce(lambda received_sum, received_amount: received_sum + sum(received_amount) if len(received_amount) > 0 else received_sum + 0, received_amounts, 0)
